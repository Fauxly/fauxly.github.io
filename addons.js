...
function deletePlugin(pluginToRemoveUrl) {
	...
	Lampa.Settings.update();
	Lampa.Noty.show("Добката е премахната, рестартирайте Lampa"); 
};
...
/* Интерфейс */
		...
		Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'TMDB Proxy',
						description: 'Постери от TMDB през прокси'
                            },
		...
		Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Отзиви',
						description: 'Добавя бутони за отзиви'
					},
					onChange: function(value) {
						if (value == '1') {
		...
		Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Приятни джаджи',
						description: 'Добавката позволява избор от различни допълнения (скринсейвъри, стил бутони, стил плеър, часовник в плеъра и др.)'
					},
					onChange: function(value) {
						if (value == '1') {
							itemON('https://andreyurl54.github.io/diesel5/tricks.js', 'Приятни джаджи', '@AndreyURL54', 'Tricks');
						}
						if (value == '2') {
		...
		Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Рейтинг от КиноПоиск и IMDB',
						description: 'Показва рейтинга от КиноПоиск и IMDB във филма. Функционира аналогично както MODSs, така че не би трябвало да ги ползвате заедно'
					},
					onChange: function(value) {
						if (value == '1') {
							itemON('https://nb557.github.io/plugins/rating.js', 'Рейтинг от КиноПоиск и IMDB', '@t_anton', 'Rating');
						}
						if (value == '2') {
	        ...
	        Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Старият стил на позициите (Отметки, харесвания, по-късно)',
						description: 'Добавката връща в главното меню старият стил на изобразяване на позициите (Отметки, харесвания, по-късно)'
					},
					onChange: function(value) {
						if (value == '1') {
							itemON('http://github.freebie.tom.ru/want.js', 'Старият стил на позициите (Отметки, харесвания, по-късно)', '@VitalikPVA', 'Want');
						}
						if (value == '2') {
	        ...
	        Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Нулиране настройките на субтитри',
						description: 'Добавката нулира всички настройки на субтитрите по подразбиране'
					},
					onChange: function(value) {
						if (value == '1') {
						       itemON('https://nb557.github.io/plugins/reset_subs.js', 'Нулиране настройките на субтитри', '@t_anton', 'Sub_reset');
						}
						if (value == '2') {
	      ...
	      Lampa.SettingsApi.addParam({
					 ...
					 description: '<div style="text-align: center;">Натиснете за презареждане на Lampa</div>',
	                           },
                                   onRender: function (item) {
                                      item.on('hover:enter', function(){
                                         location.reload();
...
/* Управление */
		...
		Lampa.SettingsApi.addParam({
            ...
            name: 'Exit_Menu',
					...
					field: {
                      name: 'Изход',
                      description: 'Добавката добавя позиция Изход в главното меню'
					},
					onChange: function(value) {
						if (value == '1') {
							itemON('https://tsynik.github.io/lampa/e.js', 'Изход', '@tsynik', 'Exit_Menu');
						}
						if (value == '2') {
		...
		Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Проверка за нови версии',
						description: 'Проверява за наличие на нови версии на приложението за Android TV'
					},
					onChange: function(value) {
						if (value == '1') {
							itemON('https://nemiroff.github.io/lampa/updater.js', 'Проверка за нови версии', '@nemiroff', 'New_Version');
							}
						if (value == '2') {
		...
		Lampa.SettingsApi.addParam({
                    ...
                    field: {
                            name: 'Бързи бутони',
                            description: 'Добавката извиква менюта на плеъра при натискане на бутони на дистанционното: 5 - плейлист, 8 - аудиопътечки, 0 - субтитри, channel+/- следващ/предишен файл в плейлистата'
                    },
					onChange: function(value) {
						if (value == '1') {
							itemON('https://nnmdd.github.io/lampa_hotkeys/hotkeys.js', 'Бързи бутони', '@nnmd', 'Hot_Buttons');
						}
						if (value == '2') {
		...
		Lampa.SettingsApi.addParam({
					...
					field: {
                                    		...
                                    		description: 'Добавката работи на устройства Orsay, за Tizen е необходимо обновяване на добавката до версия 1.9.1'
					},
					onChange: function(value) {
						if (value == '1') {
	        ...
	        Lampa.SettingsApi.addParam({
					...
					field: {
                                    		name: 'Тип на управление',
                                    		description: 'Добавката при инсталиране позволява избор на типа управление (дистанционно без мишка, дистанционно с мишка, тъчскрийн), а също така да се нулира избран по-рано тип управление'
					},
					onChange: function(value) {
						if (value == '1') {
						       itemON('https://nemiroff.github.io/lampa/select_weapon.js', 'Тип на управление', '@nemiroff', 'Select_Weapon');
						}
						if (value == '2') {
	        ...
	        Lampa.SettingsApi.addParam({
					...
					field: {
                                    		name: 'Изключване на тъч управление',
                                    		description: 'Добавката изключва сензорното управление, ако е било включено по погрешка'
					},
					onChange: function(value) {
						if (value == '1') {
...
/* Онлайн */
		...
		Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Modss',
						description: 'Добавката позволява да се гледат филми изцяло онлайн. Предлага избор от 17 балансъри и различни добавки в менюото чрез опцията Modss. VIP 4K може да се включи чрез телеграм-бота @modssmy_bot'
					},
					onChange: function(value) {
						if (value == '1') {
		...
		Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Online_Mod',
						description: 'Добавката позволява да се гледат филми и сериали изцяло онлайн. Предла избор от достъпни 7 балансъра'
					},
					onChange: function(value) {
						if (value == '1') {
		...
		Lampa.SettingsApi.addParam({
					...
					field: {
						...
						description: 'Аналог на добавката Online, но с нов информативен интрфейс, за гледане изцяло онлайн на филми и сериали'
					},
					onChange: function(value) {
						if (value == '1') {
		...
		Lampa.SettingsApi.addParam({
					...
					field: {
                                  		name: 'Онлайн',
                                  		description: 'Добавка за гледане на филми и сериали изцяло онлайн от разработчика на приложението Lampa'
					},
                           		onChange: function(value) {
					if (value == '1') {
		...
		Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Ebu.land',
						description: 'Добавка за гледане на филми и сериали във висока резолюция 4К, които са достъпни с абонамент. Подробности на сайта http://ebu.land'
					},
					onChange: function(value) {
                        			if (value == '1') {
...
/* Торрент */
		...
		Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Превключване на анализатори/парсъри',
						description: 'Добавката позволява да се превключва между jackett парсъри от списък. В настройките на анализатор/парсър ще се появи позиция с общодостъпните jackett парсъри'
					},
					onChange: function(value) {
						if (value == '1') {
							itemON('https://scabrum.github.io/plugins/jackett.js', 'Превключване на анализатори/парсъри', '@AndreyURL54', 'Switch_Parser');
						}
						if (value == '2') {
		...
		Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Tracks',
						description: 'Добавката позволява да се промени названията на аудио пътечката и субтитрите в плеъра (работи само за торенти)'
					},
					onChange: function(value) {
						if (value == '1') {
	        ...
	        Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Надстройка за торренти (Web OS, Tizen)',
						description: 'Добавка за ТВ, на които Lamap е инсталирана от официални магазини като LG Store и Tizen App Store. Включва позициите в менюото Анализатори/Парсъри и Torrserver, необходими за гледане от торенти'
					},
					onChange: function(value) {
						if (value == '1') {
							itemON('http://cub.watch/plugin/etor', 'Надстройка за торренти', '@lampa', 'Setting_torrents');
						}
						if (value == '2') {
	        ...
	        Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Търсене на локален TorrServer',
						description: 'Добавката позволява да се открият TorrServer-и в локалната мрежа'
					},
					onChange: function(value) {
						if (value == '1') {
...
/* Телеканалы */
		...
		Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Дизел ТВ',
						description: 'Добавка за безплатно гледане на телевизионни канали и платени плейлисти с телевизии и запис на архив'
					},
					onChange: function(value) {
						if (value == '1') {
							itemON('https://andreyurl54.github.io/diesel5/diesel.js', 'Дизел ТВ', '@AndreyURL54', 'Diesel');
						}
						if (value == '2') {
		...
		Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Kulik',
						description: 'Добавка за гледане на IPTV канали, сортирани по различни категории. Има възможност за промяна на стил, сървър за излъчване, а също така да се добавят канали в Избрани'
					},
					onChange: function(value) {
						if (value == '1') {
		...
		Lampa.SettingsApi.addParam({
					...
					field: {
							name: 'IPTV',
							description: 'Добавка за гледане на IPTV канали. Сортиране на каналите по групи, възможност за добавяне на каналите в Избрани. Работи само със собствени плейлисти, добавени на сайта https://cub.watch/iptv'
					},
					onChange: function(value) {
						if (value == '1') {
...
/* Радио */
		...
		Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Радио Record',
						description: 'Слущайте радио от радиостнация Radio Record. Достъпнни са повече от 50 жанра, ще се намери и за вашият вкус'
					},
					onChange: function(value) {
						if (value == '1') {
		...
		Lampa.SettingsApi.addParam({
					...
					field: {
						...
						description: 'Всичко е като Радио Record, но с качество на потока 320kbp/s и общ списък със станцииите без групиране по жанрове'
					},
					onChange: function(value) {
                        			if (value == '1') {
...
/* Клубника */
		...
		Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Ягодка',
						description: 'Добавка за гледане на 18+ съдържание'
					},
					onChange: function(value) {
						if (value == '1') {
							itemON('https://sisi.am/nyam.js', 'Ягодка', '@rik', 'Sisi');
						}
						if (value == '2') {
